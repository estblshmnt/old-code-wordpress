					...
					<?php $temp_query = $wp_query; ?>
						...
						<?php while (have_posts()) : the_post(); ?>
							...
							<?php if ( has_post_thumbnail() ) { ?>
									...
									<p class="author tabletGothic">by <?php the_author(); ?></p>
					...
					<div id="most-established-articles">
						<?php
							if (function_exists( 'wpp_get_mostpopular' )) {
							  wpp_get_mostpopular('range=weekly&order_by=views&limit=5&stats_views=0&post_type=post&stats_author=1&post_html="<li><p>{title}<span>by {author}</span></p>"');
...
-->	
	...
	<!-- 		+++++++++++++++++++++++++++Section Break++++++++++++++++++++++++ -->
		...
		<?php
$blogusers = get_users( 'number=6&exclude=1' );
// Array of WP_User objects.
foreach ( $blogusers as $user ) {  ?>
	<div class="contributor-holder">
	<?php $user_id = $user->ID; ?>
	<?php $author_url = get_author_posts_url( $user_id ); ?>
	<?php echo '<p class="contributor-name tabletGothic uppercase"><a href="'. $author_url . '">' . esc_html( $user->display_name ) . '</p>';
